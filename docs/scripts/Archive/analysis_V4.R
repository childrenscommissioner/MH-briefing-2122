...
ggsave(here('outputs','preferred_box.png'),width=5,height=5)
...
ggsave(here('outputs','no_referred_trends.png'),width=7.5,height=4)
...
ccg_df %>% 
  ...
  mutate(percreferred_202021 = round(percreferred_202021*100),
         ...
         Change = percreferred_202122-percreferred_202021) %>% 
...
# show mean and distribution of CCGs 
ccg_df %>%
...
dots outside the box and whiskers are outliers.") +
  theme(plot.caption = element_text(lineheight = 0.5, face = "italic"))
ggsave(here('outputs','refsclosed_box.png'),width=5,height=4)
...
# Note: all data sourced directly from NHS DIGITAL
...
# create df with older stats sourced from previous NHS publications
...
wait_england <- england %>% 
  select(starts_with(c("ccg","avg_wait_days"))) %>% 
  mutate(avg_wait_days_1920 = round(avg_wait_days_1920), # unrounded in NHS raw data (assume it was an oversight)
         avg_wait_days_2021 = avg_wait_days_2021) %>% 
...
wait_long_2122 <- wait_outcomes_2122 %>% 
...
wait_outcomes_2021 <- df_nhsd %>% filter(ccg %in% c("ENGLAND")) %>% 
  ...
  mutate(ccg = str_replace(ccg, "ENGLAND","England")) %>% 
  rowwise() %>% 
  mutate(within_4w = sum(wait_0to4_weeks_2021,contacts_before_referral_2021, na.rm=T),
         over_4w = sum(wait_4to6_weeks_2021, wait_6to8_weeks_2021, wait_8to10_weeks_2021, wait_10to12_weeks_2021, wait_over12_weeks_2021, na.rm=T),
         still_waiting = sum(stillwaiting_nocontact_2021,stillwaiting_onecontact_2021, na.rm=T)) %>%
  mutate(within_year = within_4w + over_4w,
...
wait_outcomes_1920 <- df_nhsd %>% filter(ccg %in% c("ENGLAND")) %>% 
  ...
  mutate(ccg = str_replace(ccg, "ENGLAND","England")) %>% 
  rowwise() %>% 
  mutate(within_4w = sum(wait_0to4_weeks_1920,contacts_before_referral_1920, na.rm=T),
         over_4w = sum(wait_4to6_weeks_1920, wait_6to8_weeks_1920, wait_8to10_weeks_1920, wait_10to12_weeks_1920, wait_over12_weeks_1920, na.rm=T),
         still_waiting = sum(stillwaiting_nocontact_1920,stillwaiting_onecontact_1920, na.rm=T)) %>%
  mutate(within_year = within_4w + over_4w,
...
wait_long_full <- wait_long_2122 %>% rbind(wait_long_2021) %>% rbind(wait_long_1920)
...
avgwait_top10 <- ccg_df %>%
  ...
  mutate(avg_wait_days_1920 = round(avg_wait_days_1920),
         avg_wait_days_2021 = round(avg_wait_days_2021),
         Change = avg_wait_days_2122- avg_wait_days_2021) %>% 
  select(ccg,avg_wait_days_2021,avg_wait_days_2122,Change) %>% 
...
spc_top10 <- ccg_df %>%
  ...
  rename('Clinical Commissioning Group'=ccg,
         'Spend per child aged 0-17 in 2020-21 (£)' = spc_202021,
         'Spend per child aged 0-17 in 2021-22 (£)' = spc_202122) %>% 
  head(n=10)
...
summary_table <- df_summary %>%
  ...
  rename('Clinical Commissioning Group'=ccg,
         'CCG overall score' = overall_score_2122,
         'Spend per child aged 0-17 (£)' = spc_202122,
         '% of budget spent on CYPMHS' = pbudget_camhs_2122,
         'Avg. wait in days' = avg_wait_days_2122,
         '% referred to CYPMHS' = percreferred_202122,
         '% referrals closed before treatment' = referrals_closed_2122) 
...
# source: https://digital.nhs.uk/data-and-information/publications/statistical/mental-health-of-children-and-young-people-in-england/2022-follow-up-to-the-2017-survey
...
# Of the 1.4 million children who have probable MH disorder what % accessed CAMHS less than half (49%) received at least 1 contact with CYPMHS and 35% received at least 2 contacts with CYPMHS.
