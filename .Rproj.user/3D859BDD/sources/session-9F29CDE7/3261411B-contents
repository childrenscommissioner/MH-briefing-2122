...
names(df_nhsd_2122) <- names_nhsd_2122$New
...
df_nhsd_2021 <- read_excel(here('inputs/Raw data','additional_cyp_wait_times_2020-21_v2.xlsx'),sheet="1b",skip=16) %>% 
  na.omit()

#import nhsd names
nhsd_names <- read.csv(here('inputs','nhsd_names.csv'))
names(df_nhsd_2021) <- nhsd_names$New

# filter out data on STPs/Region/unknowns
df_nhsd_2021 <- df_nhsd_2021 %>%  filter(str_detect(ccg, "CCG|ENGLAND"),
                                         !ccg %in% c("EAST OF ENGLAND COMMISSIONING REGION")) 
...
sum_cols_2021 <- c("no_referred_2021","wait_0to4_weeks_2021","wait_4to6_weeks_2021","wait_6to8_weeks_2021","wait_8to10_weeks_2021","wait_10to12_weeks_2021","referrals_closed_number_2021","stillwaiting_nocontact_2021","stillwaiting_onecontact_2021","contacts_before_referral_2021")
mean_cols_2021 <- c("avg_wait_days_2021", "referrals_closed_2021","overall_score_2021")


# create separate table for sum columns - summarise by ccg name 
sum_2021 <- df_nhsd_2021 %>% select(ccg, all_of(sum_cols_2021)) %>% 
  left_join(ccg_lookup %>% select(-region), by = c("ccg"="ccg")) %>% #View(.)
  unique() %>%
  group_by(ccg_new) %>% 
  mutate(across(all_of(sum_cols_2021),  ~ sum(.x, na.rm = TRUE))) %>% 
  View(.)
group_by(ccg_new) %>% 
  mutate(across(all_of(sum_cols_2021),  ~ sum(.x, na.rm = TRUE))) %>% 
  select(-ccg) %>% View(.)
